...
# Temperature15 has at least one impossible value 999 days for egg hatching
...
ggplot(frogs, aes(sample = days))+
  # stat_qq_band requires qqplotr and the latest version of qqplotr
    stat_qq_band() +
    stat_qq_line() +
    stat_qq_point() +
...
downloadthis::download_link(
  link = "https://raw.githubusercontent.com/UEABIO/5027A/main/files/frogs_messy_data.csv",
...
Check data for improper formatting, typos, missingness, duplications. {@sec-data-cleaning}

Check your data is in a tidy format. {@sec-tidy}
...
## Checking the fit of a model

After conducting a linear model analysis, we often find ourselves with results and inferences. However, before we can fully trust our analysis, it’s essential to check whether the model fits our data well. This is often described as whether our model meets certain "Assumptions" about the data.
